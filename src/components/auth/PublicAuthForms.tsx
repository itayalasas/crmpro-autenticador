    ...
    try {
      ...
      if (!result.success) {
        ...
        if (formType === 'login' && result.error?.code === 'MFA_SETUP_REQUIRED') {
          ...
          setMessage({
            type: 'success',
            text: result.error?.message || 'Debes configurar tu app Authenticator antes de continuar.'
          });
          return;
        }
          ...
          <form onSubmit={handleSubmit} className={getSpacingClass()}>
            {formType === 'login' && mfaSetupData && (
              <div className="rounded-lg border border-blue-200 bg-blue-50 p-4 space-y-3">
                ...
                <p className="text-xs text-blue-800">Tu cuenta requiere doble factor y aún no tiene un dispositivo registrado.</p>
                ...
                {Array.isArray(mfaSetupData?.setup_steps) && (
                  <ol className="text-xs text-blue-900 list-decimal pl-4 space-y-1">
                    {mfaSetupData.setup_steps.map((step: string, index: number) => (
                ...
                <p className="text-xs text-blue-700">Una vez registrado el móvil, vuelve a iniciar sesión para completar el acceso.</p>
              </div>
            )}
