    ...
    try {
      ...
      if (!result.success) {
        ...
        if (validFormType === 'login' && result.error?.code === 'MFA_SETUP_REQUIRED') {
          const token = result.data?.pairing_token;
          const setupMessage = token
            ? 'Debes configurar Authenticator antes de continuar. Token manual: ' + token
            : 'Debes configurar Authenticator antes de continuar.';
